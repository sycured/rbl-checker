...
from config import kafka_host, kafka_port, \
    ...
    kafka_ssl_certfile, kafka_ssl_keyfile, kafka_ssl_password, kafka_topic
...
def create_kafka_ssl_context() -> SSLContext:
    ...
    return create_ssl_context(
        ...
        password=kafka_ssl_password) if len(kafka_ssl_password) > 0 else \
        create_ssl_context(cafile=kafka_ssl_cafile,
                           certfile=kafka_ssl_certfile,
                           keyfile=kafka_ssl_keyfile)


async def create_aio_consumer(loop):
    """Create and return AIOKafkaProducer."""
    return AIOKafkaConsumer(
        f'{kafka_topic}', group_id='rbl_consumer', loop=loop,
